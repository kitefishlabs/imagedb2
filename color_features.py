...
class ColorFeatures:
    ...
    def default_cf_params():
        analysis_params = {
            ...
            'onset': 0,
            # (empirical) threshold for histogram; set to a positive number to remove extremely low values
            'threshold': 0,
        }
    ...
    def _analyze_image(self):
        ...
        fp = np.memmap(datapath, dtype='float32', mode='w+',
                       shape=(num_histograms, 3, ap['bins']))
        ...
        for i in range(grid_divs):
            for j in range(grid_divs):
                ...
                sub_lab = np.zeros(
                    (grid_height, grid_width, 3), dtype=np.float32)
                ...
                sub_ = img_[(i*grid_height):((i+1)*grid_height),
                            (j*grid_width):((j+1)*grid_width)]

                item = cv.calcHist([sub_], [0], None, [
                                   hist_bins], [0, hist_range])
                cv.normalize(np.where(item > thresh, item, 0), l_star)
                item = cv.calcHist([sub_], [1], None, [
                                   hist_bins], [0, hist_range])
                cv.normalize(np.where(item > thresh, item, 0), a_star)
                item = cv.calcHist([sub_], [2], None, [
                                   hist_bins], [0, hist_range])
                cv.normalize(np.where(item > thresh, item, 0), b_star)
    ...
    def get_analysis_result(self):
        ...
        datapath = os.path.abspath(
            self.analysis_params['media_file_path']+'.'+ap['color_space'])
        fp = np.memmap(datapath, dtype='float32', mode='r+',
                       shape=(((ap['grid_divs'] ** 2) + 1), 3, ap['bins']))
        return fp
