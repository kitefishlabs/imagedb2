...
class ColorFeaturesHash:
    ...
    def load_downsampled(self):
        ...
        feats = ColorFeaturesPyramid(
            **{'media_file_path': ap['media_file_path']})
        ...
        for row in range(loh):
            for col in range(low):
                ...
                binned_colors = (((dfloor(pt[2], b) << 16) + (dfloor(pt[1], b) << 8) + pt[0]), ((
                    dfloor(hipt[2], b) << 16) + (dfloor(hipt[1], b) << 8) + dfloor(hipt[0], b)))
                try:
